...
class ForceCalculator:
    ...
    def get_spring_force(self, target, neighbors, rest_lengths):
        spring_f = np.zeros(3)
        ...
        for i in range(len(neighbors)):
            ...
            spring_f += T_FINAL * self.params[0] * (rest_lengths[i]- distance) * u_target * np.heaviside(rest_lengths[i] + 0.05*R0 - distance,0)
        return spring_f
    ...
    def get_frictional_force(self, target, neighbors, contact_lengths, cortical_flow):
        ...
        for i in range(len(neighbors)):
            ...
            distance = self.dist_mat[t_idx, n_idx] #distance between target and neighbor
            u_target = self.uvec_mat[t_idx, n_idx] #unit vector from neighbor to target
            frictional_f += self.params[1] * cortical_flow * np.heaviside(contact_lengths[i] - distance,0) * (np.cross(u_target, self.rotation_axes[neighbors[i]]) - np.cross(-u_target, self.rotation_axes[target]))
        return frictional_f
    ...
    def solve_frictional_force_system(self, targets, y, radii, neighbors):
        ...
        for target in targets:
           ...
           for neighbor in neighbors_list:
                ...
                mod_mat[neighbor] = -self.params[1] * np.heaviside(contact_length - self.dist_mat[n_idx, t_idx],0) * proj_mat
