...
from ..models.model_config import T_FINAL, T_FINAL_C, NUM_EMS, R0, R0_ALT, ETA, modifiers_w_es, modifiers_wo_es
from ..models.All_Modifiers import calculate_d_cell_radius
import warnings

def objective_function(params, es_data, es_sigma, no_es_data, no_es_sigma, cvel_data, cvel_sigma):
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            
        es_pred, no_es_pred, cvel_pred = fit_model_curve((), *params)
        ...
        cvel_residuals = (cvel_data - cvel_pred)/cvel_sigma
                
        sse = np.mean(es_residuals**2) + np.mean(no_es_residuals**2) + np.mean(cvel_residuals**2)
...
def fit_model_whole(angle_data, cort_data):
    ...
    ABa_dorsal_no_es = angle_data['ABa_dorsal_avg_wo_es'].to_numpy()
    ABp_dorsal_no_es = angle_data['ABp_dorsal_avg_wo_es'].to_numpy()
    cvel_data = T_FINAL/R0_ALT * cort_data['cort_avg'].to_numpy()
    ...
    cvel_error = T_FINAL/R0_ALT * cort_data['cort_stdeofmean'].to_numpy()
    ...
    no_es_error = np.concatenate((ABa_dorsal_stdmean, ABp_dorsal_stdmean))
    ...
    def progress(xk, convergence):
        error = objective_function(xk, es_data, es_sigma, no_es_data, no_es_sigma, cvel_data, cvel_sigma)
    ...
    global_result = differential_evolution(objective_function, bounds, args = (es_data, es_sigma, no_es_data, no_es_sigma, cvel_data, cvel_sigma), disp = False, workers = 8, polish = False, callback = progress, constraints = (lc1,nlc1))
    ...
    result = minimize(objective_function, x0 = global_result.x, args = (es_data, es_sigma, no_es_data, no_es_sigma, cvel_data, cvel_sigma), method = "Nelder-Mead", bounds = bounds)
    popt = result.x
...
def fit_model_curve(x: Sequence[float], *params):
    """
    """
    sim_es = Simulator(GET_VELOCITY(params, modifiers_w_es), get_init_noise(R0, params[4], NUM_EMS, ETA))
    sim_es.run(False)

    sim_no_es = Simulator(GET_VELOCITY(params, modifiers_wo_es), get_init_noise(R0, params[4], NUM_EMS, ETA))
    sim_no_es.run(False)
    ...
    return np.concatenate((sim_es.angle["ABa_dorsal"], 
                            ...
                            sim_es.angle["ABp_ant"])), np.concatenate((sim_no_es.angle["ABa_dorsal"], sim_no_es.angle["ABp_dorsal"])), cort_vel_pred
